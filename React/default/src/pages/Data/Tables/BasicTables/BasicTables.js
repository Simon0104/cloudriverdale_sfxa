...
const BasicTables = () => {
  document.title = "Xero Data Tables | Velzon";
  ...
  useEffect(() => {
    async function fetchAll() {
      try {
        const [acc, con, inv, pay] = await Promise.all([
          axios.get('/xero/accounts'),
          axios.get('/xero/contacts'),
          axios.get('/xero/invoices'),
          axios.get('/xero/payments')
        ]);
        ...
        console.error('❌ Failed to fetch Xero data:', err.message);
      }
    }
  ...
  return (
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="Xero Integration" pageTitle="Acc SW" />
          <Row>
            <Col xl={6}>
              {renderTable(
                'Accounts',
                ['Name', 'Type'],
                data.accounts.slice(0, 5).map(a => ({ Name: a.Name, Type: a.Type }))
              )}
            </Col>
            ...
            <Col xl={6}>
              {renderTable(
                'Contacts',
                ['Name', 'Email'],
                data.contacts.slice(0, 5).map(c => ({ Name: c.Name, Email: c.EmailAddress }))
              )}
            </Col>
